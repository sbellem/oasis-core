...
func TestStatusFaults(t *testing.T) {
	...
	ns.RecordFailure(testRuntimeID, 1, maxFailures, freezeDuration)
	...
	ns.RecordFailure(testRuntimeID, 4, maxFailures, freezeDuration)
	...
	ns.RecordFailure(testRuntimeID, 9, maxFailures, freezeDuration)
	require.True(ns.IsFrozen(), "node should be frozen after reaching max failures")
	require.EqualValues(9+freezeDuration, ns.FreezeEndTime, "freeze end time should be correct")

	// Simulate node operator unfreezing in epoch 11.
	ns.Unfreeze()

	// The node should not be frozen anymore but should still be suspended.
